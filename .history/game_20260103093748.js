...
class Car {
    constructor(x, y, startAngle = 0, color = '#ff4444', playerIndex = 0, style = {}) {
    ...
    draw(ctx) {
        ...
        const roofColor = this.style.roofColor || this.getDarkerColor(this.color);
        ...
        if (shape === 'formula') {
            ...
            ctx.fillRect(-drawWidth / 2 - 6, -drawHeight / 2 - 4, drawWidth + 12, 4);
            ...
            ctx.fillStyle = roofColor;
            ctx.beginPath();
            ctx.roundRect(-drawWidth / 2 + 4, -drawHeight / 2 + 8, drawWidth - 8, drawHeight - 18, 6);
            ctx.fill();
            ...
            // Cab
            ctx.fillStyle = roofColor;
            ctx.fillRect(-drawWidth / 2, -drawHeight / 2, drawWidth * 0.6, drawHeight * 0.55);
            
            // Windshield
            ctx.fillStyle = windowColor;
            ctx.fillRect(-drawWidth / 2 + 3, -drawHeight / 2 + 4, drawWidth * 0.45, 10);
            ...
            ctx.fillStyle = roofColor;
            ctx.fillRect(-drawWidth / 2 + 3, -drawHeight / 2 + 6, drawWidth - 6, drawHeight - 18);
            
            ctx.fillStyle = windowColor;
            ctx.fillRect(-drawWidth / 2 + 4, -drawHeight / 2 + 7, drawWidth - 8, 6);
        } else {
            drawWidth = bodyWidth;
            ...
            ctx.fillStyle = roofColor;
            ctx.fillRect(-drawWidth / 2 + 3, -drawHeight / 2 + 8, drawWidth - 6, drawHeight - 20);
            
            ctx.fillStyle = windowColor;
            ctx.fillRect(-drawWidth / 2 + 4, -drawHeight / 2 + 9, drawWidth - 8, 8);
        }
        ...
        this.themes = [
            {
                ...
                background: { base: '#f4e6d2', pattern: { type: 'dots', fg: '#ead7c2', size: 40, alpha: 0.45 } },
                track: { base: '#b57a45', edge: '#7e4f2d', stripe: '#fff3de', dash: [18, 10], texture: { type: 'grain', fg: '#c58d5a', size: 16, alpha: 0.35 } },
                checkpoint: '#ffd24a',
                vehicle: { shape: 'roadster' }
            },
            {
                name: 'Pool Table',
                background: { base: '#0f5a3a', pattern: { type: 'grid', fg: '#134f35', size: 48, alpha: 0.4 } },
                track: { base: '#15452c', edge: '#0c2f1e', stripe: '#f5f2e8', dash: [14, 12], texture: { type: 'speckles', fg: '#1c5a39', size: 20, alpha: 0.4 } },
                checkpoint: '#f4c95d',
                vehicle: { shape: 'formula' }
            },
            {
                name: 'Office Desk',
                background: { base: '#dad4cc', pattern: { type: 'grid', fg: '#c6beb3', size: 36, alpha: 0.5 } },
                track: { base: '#4f5258', edge: '#2f3236', stripe: '#f0f0f0', dash: [22, 12], texture: { type: 'stripes', fg: '#5d6168', size: 24, alpha: 0.35 } },
                checkpoint: '#ffcc66',
                vehicle: { shape: 'compact' }
            },
            {
                name: 'Garden Path',
                background: { base: '#6d8b57', pattern: { type: 'speckles', fg: '#5a7648', size: 28, alpha: 0.45 } },
                track: { base: '#8b643a', edge: '#5f4325', stripe: '#f4e7d2', dash: [16, 14], texture: { type: 'speckles', fg: '#9a7347', size: 18, alpha: 0.4 } },
                checkpoint: '#ffe082',
                vehicle: { shape: 'buggy' }
            },
            {
                name: 'Bathtub Ring',
                background: { base: '#e7f0f2', pattern: { type: 'waves', fg: '#d3e2e6', size: 48, alpha: 0.45 } },
                track: { base: '#c3c9cd', edge: '#9ea6ab', stripe: '#ffffff', dash: [20, 14], texture: { type: 'stripes', fg: '#d5dadf', size: 30, alpha: 0.35 } },
                checkpoint: '#ffe17a',
                vehicle: { shape: 'soapboat' }
            },
            {
                name: 'Bookshelf Slalom',
                background: { base: '#7a5b3a', pattern: { type: 'planks', fg: '#6b4d32', size: 60, alpha: 0.45 } },
                track: { base: '#5a3f2a', edge: '#3d2a1b', stripe: '#f3e5cf', dash: [16, 10], texture: { type: 'grain', fg: '#6b4b30', size: 18, alpha: 0.4 } },
                checkpoint: '#ffd66e',
                vehicle: { shape: 'truck' }
            },
            {
                name: 'Treehouse',
                background: { base: '#597447', pattern: { type: 'speckles', fg: '#4a643c', size: 30, alpha: 0.45 } },
                track: { base: '#a0744e', edge: '#6b4b32', stripe: '#fff1d6', dash: [18, 12], texture: { type: 'planks', fg: '#8a6440', size: 40, alpha: 0.4 } },
                checkpoint: '#ffe08c',
                vehicle: { shape: 'truck' }
            },
            {
                name: 'Toy Box',
                background: { base: '#ffe5c2', pattern: { type: 'confetti', colors: ['#ff6b6b', '#2ec4b6', '#ffd166', '#5fa8ff'], size: 50, alpha: 0.5 } },
                track: { base: '#ff7f50', edge: '#c85c38', stripe: '#ffffff', dash: [12, 10], texture: { type: 'dots', fg: '#ff966d', size: 18, alpha: 0.4 } },
                checkpoint: '#fff1a1',
                vehicle: { shape: 'compact' }
            },
            {
                name: 'Kitchen Counter',
                background: { base: '#e9ecef', pattern: { type: 'tiles', fg: '#cfd6dd', size: 52, alpha: 0.5 } },
                track: { base: '#7b8086', edge: '#5b5f64', stripe: '#ffffff', dash: [20, 12], texture: { type: 'speckles', fg: '#90959b', size: 16, alpha: 0.45 } },
                checkpoint: '#ffdd77',
                vehicle: { shape: 'roadster' }
            },
            {
                name: 'Dinner Plate',
                background: { base: '#f5f1e6', pattern: { type: 'rings', fg: '#e0d8cc', size: 80, alpha: 0.35 } },
                track: { base: '#c23b22', edge: '#8f2b18', stripe: '#fff0d8', dash: [14, 8], texture: { type: 'speckles', fg: '#d34a2f', size: 14, alpha: 0.4 } },
                checkpoint: '#ffe7a1',
                vehicle: { shape: 'formula' }
            }
        ];
        ...
        const applyFeature = () => {
            ...
            switch (feature) {
                case 0: {
                    ...
                    const start = 6;
                    [180, -180, 140, -140].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 1: {
                    // Hairpin: tight, sharp turn
                    const start = 11;
                    [-260, -340, -260].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 2: {
                    // Kink: subtle jog on a straight
                    const start = 8;
                    [120, -80].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 3: {
                    // Esses: flowing S curve
                    const start = 9;
                    [140, 100, -120, -160].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 4: {
                    // Carousel: long, tightening curve
                    const start = 13;
                    [120, 180, 220, 180, 120].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 5: {
                    // Slalom: repeated quick direction changes
                    const start = 7;
                    [140, -140, 140, -140, 120, -120].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 6: {
                    // Switchback: double hairpin feel
                    const start = 12;
                    [-220, -300, -220, 220, 300, 220].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 7: {
                    // Sweeper: big, fast arc
                    const start = 15;
                    [120, 160, 200, 220, 200, 160, 120].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 8: {
                    // Bus stop chicane: sharper, closer offsets
                    const start = 10;
                    [200, -220, 200].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                case 9: {
                    // Dogleg + pinch: small offset then tighter bend
                    const start = 14;
                    [120, 180, 240].forEach((offset, i) => applyLateralOffset(start + i, offset));
                    break;
                }
                default:
                    break;
...
class Game {
    ...
    initCars() {
        ...
        for (let i = 0; i < this.playerCount; i++) {
            ...
            this.cars.push(new Car(x, y, startAngle, this.playerColors[i], i, style));
        }
    }
    ...
    resolveCarCollisions() {
        ...
        for (let i = 0; i < this.cars.length; i++) {
            for (let j = i + 1; j < this.cars.length; j++) {
                ...
                const minDist = carA.getCollisionRadius() + carB.getCollisionRadius();
                ...
                carA.speed = (newVelAX * forwardAX + newVelAY * forwardAY) * speedDamp;
                carB.speed = (newVelBX * forwardBX + newVelBY * forwardBY) * speedDamp;
                
                carA.speed = Math.max(Math.min(carA.speed, carA.getCurrentMaxSpeed()), -carA.getCurrentMaxSpeed() * 0.5);
                carB.speed = Math.max(Math.min(carB.speed, carB.getCurrentMaxSpeed()), -carB.getCurrentMaxSpeed() * 0.5);
            }
        }
    }
